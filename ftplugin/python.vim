...
command! -nargs=+ NC call PyNC(<f-args>)
command! -nargs=1 PI call PyInt(<f-args>)
command! -nargs=1 NT call PyNewTest(<f-args>)
command! PYM call PyMainTemplate()


augroup python
   au!
   au Filetype python call Filetype_Python() |
      \ compiler pyunit
   au BufNewFile *.py call Py_template("python")
   "au FileType python menu Tags.python :!ctags --recurse --language-force=Python --extra=+q --fields=+i *.py<CR>
   "You can use <c-j> to goto the next <++> - it is pretty smart ;)
   "au BufNewFile,BufRead *.py inorea <buffer> cfun <c-r>=IMAP_PutTextWithMovement("def <++>(<++>):\n<++>\nreturn <++>")<cr>
   "au BufRead,BufNewFile *.py inorea <buffer> cclass <c-r>=IMAP_PutTextWithMovement("class <++>:\n<++>")<cr>
   "au BufRead,BufNewFile *.py inorea <buffer> cfor <c-r>=IMAP_PutTextWithMovement("for <++> in <++>:\n<++>")<cr>
   "au BufRead,BufNewFile *.py inorea <buffer> cif <c-r>=IMAP_PutTextWithMovement("if <++>:\n<++>")<cr>
   "au BufRead,BufNewFile *.py inorea <buffer> cifelse <c-r>=IMAP_PutTextWithMovement("if <++>:\n<++>\nelse:\n<++>")<cr>
augroup END
